...
const MessagesScreen = () => {
  ...
  return (
    <Container className="p-2">
      ...
      <Modal show={showProfileLink} onHide={() => setShowProfileLink(false)}>
        <Modal.Header closeButton>
          <Modal.Title>Share your profile</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <p>
            Share following link with others and have them post your message on
            your profile anonymously.
          </p>
          <Form>
            <Form.Group>
              <Form.Row>
                <Col sm={10}>
                  <Form.Control
                    type="text"
                    value={`${window.location.hostname}/profile/${userInfo.username}`}
                    readOnly
                  />
                </Col>
                <Col>
        ...
        <Modal.Footer>
          <Button variant="dark" onClick={() => setShowProfileLink(false)}>Close</Button>
        </Modal.Footer>
      </Modal>
      <hr />
      {isFetching ? (
